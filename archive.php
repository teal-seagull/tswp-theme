<?php get_header(); ?>
        ...
        <div id="container">
            <div id="content">
				...
				<?php if ( is_day() ) : ?>
				                <h1 class="page-title"><?php printf( __( 'Daily Archives: <span>%s</span>', 'tealseagull' ), get_the_time(get_option('date_format')) ) ?></h1>
				<?php elseif ( is_month() ) : ?>
				                <h1 class="page-title"><?php printf( __( 'Monthly Archives: <span>%s</span>', 'tealseagull' ), get_the_time('F Y') ) ?></h1>
				<?php elseif ( is_year() ) : ?>
				                <h1 class="page-title"><?php printf( __( 'Yearly Archives: <span>%s</span>', 'tealseagull' ), get_the_time('Y') ) ?></h1>
				<?php elseif ( isset($_GET['paged']) && !empty($_GET['paged']) ) : ?>
				                <h1 class="page-title"><?php _e( 'Blog Archives', 'tealseagull' ) ?></h1>
				<?php endif; ?>
				...
				<?php global $wp_query; $total_pages = $wp_query->max_num_pages; if ( $total_pages > 1 ) { ?>
				                <div id="nav-above" class="navigation">
				                    <div class="nav-previous"><?php next_posts_link(__( '<span class="meta-nav">&laquo;</span> Older posts', 'tealseagull' )) ?></div>
				                    <div class="nav-next"><?php previous_posts_link(__( 'Newer posts <span class="meta-nav">&raquo;</span>', 'tealseagull' )) ?></div>
				                </div><!-- #nav-above -->
				<?php } ?>            
				...
				<?php the_excerpt( __( 'Continue reading <span class="meta-nav">&raquo;</span>', 'tealseagull' )  ); ?>
				                    </div><!-- .entry-summary -->
